...
recoded_data <- selected_data |> 
  ...
  mutate(
    ...
    qa1 = factor(qa1, ordered = FALSE,
                 labels = c("Yes a lot", "Somewhat", "Not really", "Not at all")
    ),
    ...
    d63 = factor(d63, ordered = F,
                 labels = c("working", "lwr-mid", "middle", "upr-mid", "higher")
    ),
...
```{r}
library(lme4)
mixed_model1 <- glmer(qc19 ~ d70 + polintr + d63 + qa1 +
    (1 | COUNTRY), data = training.data, family = binomial, 
